...
let fail message = raise_s [%sexp (message : string)]
...
let require_equal_int actual expect =
  if actual <> expect
  then raise_s [%sexp "ints differ", { actual : int; expect : int }]
;;

let require_equal_bool actual expect =
  if Bool.(actual <> expect)
  then raise_s [%sexp "bools differ", { actual : bool; expect : bool }]
;;

let require_one_background_command commands =
  match commands with
  | [ { Todos.Command.target = Background; request } ] -> request
  | _ -> raise_s [%sexp "expected one background command"]
;;

let require_no_commands commands =
  match commands with
  | [] -> ()
  | _ -> raise_s [%sexp "expected no commands"]
;;
...
let test_load_schedules_background_query () =
  let model, commands = Todos.Model.update Todos.Model.initial Todos.Action.Load in
  ...
  | Load_all -> ()
  | Persist _ | Subscribe_query _ | Unsubscribe_query _ -> fail "load must not issue a write"
;;

let test_submit_trims_and_schedules_background_write () =
  let model = { Todos.Model.initial with draft = "  Ship cross-platform todos  " } in
  let model, commands =
    Todos.Model.update
      model
...
let test_blank_submit_is_ignored () =
  ...
  let model, commands =
    Todos.Model.update model (Todos.Action.Submit_new { id = "ignored"; created_at_ms = 1 })
  in
  require_equal_string model.draft "   ";
...
let test_toggle_and_delete_schedule_background_writes () =
  ...
  let model, _ = Todos.Model.update Todos.Model.initial (Todos.Action.Loaded [ existing ]) in
  let _, toggle_commands = Todos.Model.update model (Todos.Action.Toggle "todo-1") in
  (match require_one_background_command toggle_commands with
   | Persist (Toggle "todo-1") -> ()
   | _ -> fail "toggle must issue a toggle write");
  let _, delete_commands = Todos.Model.update model (Todos.Action.Delete "todo-1") in
...
let test_loaded_and_failed_update_bonsai_owned_state () =
  let loaded =
    [ todo ~id:"todo-2" ~title:"Desktop UI" ~created_at_ms:20 ()
    ; todo ~id:"todo-1" ~title:"iOS UI" ~created_at_ms:10 ()
    ]
  in
  let model, commands = Todos.Model.update Todos.Model.initial (Todos.Action.Loaded loaded) in
  require_no_commands commands;
  require_equal_bool model.is_loading false;
  require_todos model.todos loaded;
  let model, commands = Todos.Model.update model (Todos.Action.Store_failed "sqlite failed") in
  require_no_commands commands;
  require_equal_bool model.is_loading false;
...
let test_datascript_store_roundtrip () =
  ...
  let store = empty () in
  let store = apply_write store (Add (todo ~id:"todo-1" ~title:"iOS app" ~created_at_ms:10 ())) in
  let store = apply_write store (Add (todo ~id:"todo-2" ~title:"Mac app" ~created_at_ms:20 ())) in
...
let test_storage_codec_encodes_datascript_payloads_as_transit () =
  ...
  let storage : Ds.storage =
    ...
    ; storage_delete =
        (fun addresses ->
          List.iter addresses ~f:(fun address -> Hashtbl.remove encoded_payloads address))
    }
  in
  ...
  if not (String.is_prefix root_payload ~prefix:"[\"^ \"")
  then fail "storage payload must be transit JSON"
;;
...
let test_screen_model_filters_routes_search_and_selection () =
  ...
  require_todos screen.visible_todos [ List.nth_exn todos 0; List.nth_exn todos 1 ];
  ...
  let active_screen =
    Todos.Screen.create
      model
      ~route:Todos.Screen.Route.Active
      ~search:""
      ~selected_todo_id:""
  in
  require_equal_string active_screen.title "Active";
  require_todos active_screen.visible_todos [ List.nth_exn todos 0; List.nth_exn todos 2 ];
  let completed_screen =
    Todos.Screen.create
      model
      ~route:Todos.Screen.Route.Completed
      ~search:""
      ~selected_todo_id:""
  in
...
let test_sqlite_storage_roundtrip () =
  ...
  let store =
    apply_write store (Add (todo ~id:"todo-1" ~title:"Persist me" ~created_at_ms:100 ()))
  in
  require_todos
    (list store)
...
let test_runtime_notifies_query_subscribers_after_write () =
  ...
  Todos.Runtime.subscribe_query
    ~path:db_path
    ~id:"test"
    ~query:Todos.Query.List_todos
    ~on_change:(fun action -> received := action :: !received);
  (match !received with
   | [ Loaded [] ] -> ()
   | _ -> fail "subscription must receive initial query result");
  let first = todo ~id:"todo-1" ~title:"Subscribed write" ~created_at_ms:700 () in
  (match
     Todos.Runtime.execute_command
       ~path:db_path
...
let () =
  ...
  ; "Runtime executes commands against SQLite", test_runtime_executes_commands_against_sqlite
